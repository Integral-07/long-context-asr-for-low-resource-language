...
def decode_beams_lm(
    ...
    for logits, length in zip(logits_list, encoded_lengths):
        ...
        text_frames= b.text_frames if hasattr(b, 'text_frames') else b[2]
        logit_score= b.logit_score if hasattr(b, 'logit_score') else b[3]
        lm_score   = b.lm_score    if hasattr(b, 'lm_score')    else b[4]

        decoded_data.append({
                'text': text,
                'frames': [proc_text_frame(el) for el in text_frames] if ds_factor != None else None,
                'ngram_score': lm_score - logit_score,
                'am_score': logit_score,
                'score': lm_score, # # score = ngram_score + am_score
        })
...
def fetch_logits(args, model:SCConformerXL, spec:torch.Tensor, seq_len:int, overlap:int, tokenizer, use_tqdm=True):
    ...
    for i in pbar:
        ...
        key = 'final_posteriors' if 'final_posteriors' in out else 'final_posteriors_ctc'
        logits = out[key].detach().cpu()
	# convert to prob
